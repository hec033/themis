...
def extract_encode_law_content(openai_client, url: str):
    ...
    section_dict = {
        "metadata": {
            "article": soup.find(class_='nys-openleg-result-nav-bar-item nys-openleg-result-nav-bar-item-up').find(
                class_="nys-openleg-result-nav-item-name").text.strip(),
            "article_title": soup.find(
                class_='nys-openleg-result-nav-bar-item nys-openleg-result-nav-bar-item-up').find(
                class_="nys-openleg-result-nav-item-description").text.strip(),
            "section_number": soup.find_all("div", {"class": "nys-openleg-result-title-headline"})[0].text.strip(),
            "section_title": soup.find_all("div", {"class": "nys-openleg-result-title-short"})[0].text.strip(),
            "section_title_location": soup.find_all("div", {"class": "nys-openleg-result-title-location"})[
                0].text.strip()
        },
    ...
    metadata_hashable = section_dict["metadata"]["article"] + '-' + section_dict["metadata"]["article_title"] + '-' + \
                        section_dict["metadata"]["section_number"] + \
                        '-' + section_dict["metadata"]["section_title"] + '-' + section_dict["metadata"][
